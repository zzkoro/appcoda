...
struct DashboardView: View {
    ...
    @FetchRequest(entity: PaymentActivity.entity(), sortDescriptors: [ NSSortDescriptor(keyPath: \PaymentActivity.date, ascending: false)])
    ...
    private var paymentDataForView: [PaymentActivity] {
        switch listType {
        case .all:
            return paymentActivities.sorted(by: { $0.date.compare($1.date) == .orderedDescending})
        case .income:
...
struct DashboardView_Previews: PreviewProvider {
    ...
    static var previews: some View {
        ...
        return Group {
            ...
            .previewDisplayName("MenuBar")
            IncomeCard().previewLayout(.sizeThatFits).previewDisplayName("IncomeCard")
            ExpenseCard().previewLayout(.sizeThatFits).previewDisplayName("ExpenseCard")
            TotalBalanceCard().previewLayout(.sizeThatFits).previewDisplayName("TotalBalanceCard")
            TransactionHeader(listType: .constant(.all)).previewLayout(.sizeThatFits).previewDisplayName("TransactionHeader")
            TransactionCellView(transaction: testTrans).previewLayout(.sizeThatFits).previewDisplayName("TransactionCell")
